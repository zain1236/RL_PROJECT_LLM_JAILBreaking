...
def load_agent_for_ppo(model_name="EleutherAI/pythia-1.4b"):
    ...
    # eos stays at whatever Pythia sets (token 0) — we never pass it to generate
    print(f"Tokenizer pad_token_id : {tokenizer.pad_token_id}")
    print(f"Tokenizer eos_token_id : {tokenizer.eos_token_id}")

    # Use float32 — float16 causes NaN logits on nightly PyTorch + Pythia
...
def generate_jailbreak_prompt(model, tokenizer, target_string, device, max_new_tokens=60):
    ...
    # Greedy decoding — avoids torch.multinomial entirely (no CUDA assert possible)
    with torch.no_grad():
        output_ids = model.generate(
            input_ids,
            attention_mask = attention_mask,
            max_new_tokens = max_new_tokens,
            do_sample      = False,          # greedy — no multinomial, no assert
            pad_token_id   = tokenizer.pad_token_id,
            eos_token_id   = tokenizer.pad_token_id,  # use pad as eos to avoid token-0 stop
        )
